...
export function findToolPathMatches(toolName: string, input: unknown, cwd: string): DangerousMatch[] {
...
export function formatToolRequest(toolName: string, input: unknown): string {
...
function collectPathMatches(rawPath: string, cwd: string, access: PathAccess): DangerousMatch[] {
	...
	if (access === "write" && SYSTEM_WRITE_DIRS.some((dir) => isPathInsideOrEqual(resolvedPath, dir) || isPathInsideOrEqual(lexicalPath, dir))) {
		addMatch(matches, {
			id: "system-write",
...
function packageInstallRunsScripts(command: string, subcommand: string, args: string[]): boolean {
	...
	if (command === "pip" || command === "pip3") return ["install", "download"].includes(subcommand);
	if (command === "pipx") return ["install", "upgrade", "run"].includes(subcommand);
	if (command === "uv") {
		if (["add", "sync"].includes(subcommand)) return true;
		if (subcommand === "pip") return args.includes("install") || args.includes("download");
		return false;
	}
	if (command === "cargo") return subcommand === "install";
	if (command === "gem") return ["install", "update"].includes(subcommand);
	return false;
}
...
function hasDangerousContainerRunFlag(args: string[]): boolean {
	for (let index = 0; index < args.length; index++) {
		...
		const cap = containerOptionValue(arg, previous, "--cap-add");
		if (cap && UNSAFE_CONTAINER_CAPS.has(cap.toUpperCase().replace(/^CAP_/, ""))) return true;
...
export function formatReasons(matches: DangerousMatch[]): string {
